...
def schwefel_func(x, nx, Os, Mr, s_flag, r_flag):
    ...
    for i in range(nx):
        ...
        if z[i] > 500:
            f -= (500.0-np.fmod(z[i], 500)) * \
                np.sin(pow(500.0-np.fmod(z[i], 500), 0.5))
            tmp = (z[i]-500.0)/100
            f += tmp*tmp/nx
        elif (z[i] < -500):
            f -= (-500.0 + np.fmod(np.fabs(z[i]), 500)) * \
                np.sin(pow(500.0-np.fmod(np.fabs(z[i]), 500), 0.5))
            tmp = (z[i]+500.0)/100
            f += tmp*tmp/nx
        else:
...
def hgbat_func(x, nx, Os, Mr, s_flag, r_flag):
    ...
    f = pow(np.fabs(pow(r2, 2.0)-pow(sum_z, 2.0)),
            2*alpha) + (0.5*r2 + sum_z)/nx + 0.5
    return f
...
def cf01(x, nx, Os, Mr, s_flag, r_flag):
    ...
    i = 0
    fit[i] = rosenbrock_func(x, nx, Os[i*nx:(i+1)*nx],
                             Mr[i*nx:(i+1)*nx, 0:nx], 1, r_flag)
    fit[i] = 10000*fit[i]/1e+4
    i = 1
    fit[i] = ellips_func(x, nx, Os[i*nx:(i+1)*nx],
                         Mr[i*nx:(i+1)*nx, 0:nx], 1, r_flag)
    fit[i] = 10000*fit[i]/1e+10
    i = 2
    fit[i] = bent_cigar_func(x, nx, Os[i*nx:(i+1)*nx],
                             Mr[i*nx:(i+1)*nx, 0:nx], 1, r_flag)
    fit[i] = 10000*fit[i]/1e+30
    i = 3
    fit[i] = discus_func(x, nx, Os[i*nx:(i+1)*nx],
                         Mr[i*nx:(i+1)*nx, 0:nx], 1, r_flag)
    fit[i] = 10000*fit[i]/1e+10
    i = 4
    fit[i] = ellips_func(x, nx, Os[i*nx:(i+1)*nx],
                         Mr[i*nx:(i+1)*nx, 0:nx], 1, 0)
    fit[i] = 10000*fit[i]/1e+10
...
def cf02(x, nx, Os, Mr, s_flag, r_flag):
    ...
    i = 0
    fit[i] = schwefel_func(x, nx, Os[i*nx:(i+1)*nx],
                           Mr[i*nx:(i+1)*nx, 0:nx], 1, 0)
    i = 1
    fit[i] = rastrigin_func(x, nx, Os[i*nx:(i+1)*nx],
                            Mr[i*nx:(i+1)*nx, 0:nx], 1, r_flag)
    i = 2
    fit[i] = hgbat_func(x, nx, Os[i*nx:(i+1)*nx],
                        Mr[i*nx:(i+1)*nx, 0:nx], 1, r_flag)
    f = cf_cal(x, nx, Os, delta, bias, fit, cf_num)
    return f
...
def cf06(x,  nx, Os, Mr, s_flag, r_flag):
    ...
    i = 0
    fit[i] = escaffer6_func(x, nx, Os[i*nx:(i+1)*nx],
                            Mr[i*nx:(i+1)*nx, 0:nx], 1, r_flag)
    ...
    fit[i] = schwefel_func(x, nx, Os[i*nx:(i+1)*nx],
                           Mr[i*nx:(i+1)*nx, 0:nx], 1, r_flag)
    i = 2
    fit[i] = griewank_func(x, nx, Os[i*nx:(i+1)*nx],
                           Mr[i*nx:(i+1)*nx, 0:nx], 1, r_flag)
...
def getAbsolutePath(neededFilePath):
