...
def mfea(tasks,
    ...
    for rep in range(reps):
        ...
        population = np.asarray([Individual(D_multitask, tasks)
                                for _ in range(2*pop)])
        factorial_costs = np.full(
            shape=(2 * pop, no_of_tasks), fill_value=np.inf)
        ...
        for i, individual in enumerate(population[:pop]):
            ...
            j, factorial_cost, calls_per_individual[i] = individual.evaluate(
                p_il, method)
        ...
        for generation in range(gen):
            ...
            if selection_process == 'elitist':
                scalar_fitnesses = np.array(
                    [individual.scalar_fitness for individual in population])
                ...
                scalar_fitnesses = np.array(
                    [individual.scalar_fitness for individual in population])
                for i in range(pop):
                    population[i] = population[RouletteWheelSelection(
                        scalar_fitnesses)]
